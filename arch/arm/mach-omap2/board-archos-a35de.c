...
#ifdef CONFIG_PM
struct vout_platform_data a35de_vout_data = {
...
{
	if ( hardware_rev > 1 ) {
		// dc_detect is pmic's gpio_0 - as on the other products
...
static int dcx78_init(void)
{
	...
	/* pull the DCX78 out of reset */
	gpio_request(160, "de_reset");
	omap_cfg_reg(T21_3630_GPIO160);
	gpio_direction_output(160, 0);
	
	return 0;
}

static int gpio_dc_detect_setup(void)
{
	// dc_detect on v1.1 boards
	if ( hardware_rev == 0 || hardware_rev == 1 )
		omap_cfg_reg(AG8_3430_GPIO28);
...
MACHINE_START(ARCHOS_A35DE, "Archos A35DE board")
	.phys_io	= 0x48000000,
