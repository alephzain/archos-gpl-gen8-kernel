...
static ssize_t display_vcom_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	vcom = dssdev->get_vcom(dssdev);

	return snprintf(buf, PAGE_SIZE, "0x%05x\n", vcom);
}

static ssize_t display_vcom_store(struct device *dev,
		...
		const char *buf, size_t size)
{
	...
	if (!dssdev->get_vcom || !dssdev->set_vcom)
		return -ENOENT;

	if (strict_strtoul(buf, 0, &vcom))
		return -EINVAL;

	if (vcom > 0xff)
		return -EINVAL;

	r = dssdev->set_vcom(dssdev, vcom);
	if (r)
		return r;
...
static DEVICE_ATTR(vcom, S_IRUGO|S_IWUSR,
		display_vcom_show, display_vcom_store);
