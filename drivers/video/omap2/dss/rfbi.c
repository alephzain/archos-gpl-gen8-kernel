...
{
	int rfbi_module;
	enum omap_dss_update_mode current_mode;

	rfbi_module = rfbi_find_display(dssdev);
	current_mode = rfbi.update[rfbi_module].mode;
...
{
	DSSDBG("rfbi_display_enable_te\n");

	dssdev->driver->enable_te(dssdev, enable);

	if (enable)
		omap_rfbi_setup_te(OMAP_DSS_RFBI_TE_MODE_1,
			     2000000, 50000000, 0, 0, 0);
	omap_rfbi_enable_te( enable, 0);
	return 0;
}

static int rfbi_display_get_te(struct omap_dss_device *dssdev)
{
	return rfbi.te_enabled;
}
...
static int rfbi_display_enable(struct omap_dss_device *dssdev)
{
	...
	if (dssdev->phy.rfbi.ext_te)
		rfbi_display_enable_te(dssdev, dssdev->phy.rfbi.ext_te);
...
int rfbi_init_display(struct omap_dss_device *dssdev)
{
	...
	dssdev->get_te = rfbi_display_get_te;
	...
	rfbi.update_delay = msecs_to_jiffies(33);
