...
/* ST7791 magic registers */
#define ST7781_REG_D1				0xD1
#define ST7781_EEPROM_ID			0xD2
#define ST7781_EN_EEPROM			0xFA
#define ST7781_EN_FAHFEH			0xFF
...
{  
	...
	ili932x_write_reg(dev, ILI9325_DRV_OUTPUT_CTRL_1, 0x0100);	// Output Direct
	ili932x_write_reg(dev, ILI9325_LCD_DRV_CTRL, 0x0700);		// Line Inversion

	//Entry Mode	18 bits in 2 transfert ( 16 + 2) tri=1 dfm=0
	ili932x_write_reg(dev, ILI9325_ENTRY_MOD, 0x9030);
	ili932x_write_reg(dev, ILI9325_RESIZE_CTRL, 0x0000);		// Resize register (no resizing)
	ili932x_write_reg(dev, ILI9325_DIS_CTRL_1, 0x0121);		// Display Control1
	ili932x_write_reg(dev, ILI9325_DIS_CTRL_2, 0x0302);		// Porch setting
	ili932x_write_reg(dev, ILI9325_DIS_CTRL_3, 0x0200);		// Scan Cycle
	ili932x_write_reg(dev, ILI9325_DIS_CTRL_4, 0x0000);		// FMARK off

	//---- Power Control Registers Initial ----//
	ili932x_write_reg(dev, ILI9325_POW_CTRL_1, 0x0790);		// Power Control1
	ili932x_write_reg(dev, ILI9325_POW_CTRL_2, 0x0005);		// Power Control2
	ili932x_write_reg(dev, ILI9325_POW_CTRL_3, 0x0000);		// Power Control3
	ili932x_write_reg(dev, ILI9325_POW_CTRL_4, 0x0000);		// Power Control4
	msleep(100);

	//---- Power Supply Startup 1 Setting-----//
	ili932x_write_reg(dev, ILI9325_POW_CTRL_1, 0x1490);		// Power Control1
	msleep(50);

	ili932x_write_reg(dev, ILI9325_POW_CTRL_2, 0x0227);		// Power Control2
	msleep(50);

	//---- Power Supply Startup 2 Setting-----//
	ili932x_write_reg(dev, ILI9325_POW_CTRL_3, 0x0089);		// Power Control3
	ili932x_write_reg(dev, ILI9325_POW_CTRL_4, 0x1900);		// Power Control4
	ili932x_write_reg(dev, ILI9325_POW_CTRL_7, 0x001a);		// VCOMH setting
	msleep(50);

	st7781_gamma_adjust(dev);

	//-----Display Windows 240 X 320-----------//
	ili932x_write_reg(dev, ILI9325_HOR_ADDR_START, 0x0000);	// Horizontal Address Start Position
	ili932x_write_reg(dev, ILI9325_HOR_ADDR_END, 0x00ef);	// Horizontal Address End Position
	ili932x_write_reg(dev, ILI9325_VET_ADDR_START, 0x0000);	// Vertical Address Start Position
	ili932x_write_reg(dev, ILI9325_VET_ADDR_END, 0x013f);	// Vertical Address End Position

	//------Frame Rate Setting--------------------//
	ili932x_write_reg(dev, ILI9325_DRV_OUTPUT_CTRL_2, 0xa700);	// Gate scan control
	ili932x_write_reg(dev, ILI9325_BASE_IMG_CTRL, 0x0001);		// Non-display Area setting

	ili932x_write_reg(dev, ILI9325_PAN_CTRL_1, 0x0033);		// RTNI setting
	ili932x_write_reg(dev, ILI9325_DIS_CTRL_3, 0x0000);		// Scan Cycle

	//------Microtek magic tweak --------------------//
	ili932x_write_reg(dev, ST7781_EN_FAHFEH, 0x0001);
	ili932x_write_reg(dev, ST7781_EN_EEPROM, 0x0002);
	ili932x_write_reg(dev, ST7781_REG_D1, 0x0064);
	ili932x_write_reg(dev, ST7781_EEPROM_ID, 0x0085);
	ili932x_write_reg(dev, ST7781_EN_FAHFEH,0x0000);

	ili932x_write_reg(dev, ILI9325_DIS_CTRL_1, 0x0133);
...
{
	*width = 43;			/* display width in mm  */
	*height = 57;			/* display height in mm */
}
